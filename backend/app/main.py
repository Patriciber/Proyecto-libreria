...
# Rutas absolutas para independencia del CWD
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.dirname(CURRENT_DIR)  # Carpeta 'backend'
...
async def read_analisis():
    ...
    analisis_path = os.path.join(PROJECT_ROOT, "backend", "docs", "analisis_final_interactivo.html")
    return FileResponse(analisis_path)
...
# Servir archivos estáticos del frontend desde la carpeta vecina (ruta absoluta)
frontend_path = os.path.join(PROJECT_ROOT, "frontend")
app.mount("/", StaticFiles(directory=frontend_path, html=True), name="static")

# Alias para compatibilidad con el archivo de arranque raíz
hexagonal_app = app
